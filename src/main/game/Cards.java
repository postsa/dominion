...
public class Cards {
  ...
  public int count(String cardName) {
    long count =
        this.victory.stream().filter(c -> cardName.equals(c.getName())).count()
  ...
  public void put(Cards otherCards) {
    while (!otherCards.getTreasure().empty()) this.treasure.push(otherCards.getTreasure().pop());
  ...
  public Cards take(ActionCard actionCard) {
    ...
    Optional<ActionCard> anyActionCard =
        this.actionCards.stream().filter(c -> actionCard.getName().equals(c.getName())).findFirst();
  ...
  public Cards take(TreasureCard treasureCard) {
    Cards takenCards = new Cards();
    Optional<TreasureCard> anyTreasureCard =
        this.treasure.stream().filter(c -> treasureCard.getName().equals(c.getName())).findFirst();
  ...
  public Cards take(VictoryCard victoryCard) {
    Cards takenCards = new Cards();
    Optional<VictoryCard> anyVictoryCard =
        this.victory.stream().filter(c -> victoryCard.getName().equals(c.getName())).findFirst();
