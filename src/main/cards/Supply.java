...
public class Supply extends CardInventory implements CardMerchant {
  ...
  public Purchasable requestCardInfo(String cardName) {
    ...
    if (cardInfo == null)
      throw new NoSuchCardAvailable(
          String.format("Cannot request card info for unregistered card '%s'.", cardName));
    return cardInfo;
  }
  ...
  public DeckOfCards getStartingDeck() {
    ...
    for (int i = 0; i < 3; i++) startingDeck.add(estate);
    for (int i = 0; i < 7; i++) startingDeck.add(copper);
    return startingDeck;
  }
